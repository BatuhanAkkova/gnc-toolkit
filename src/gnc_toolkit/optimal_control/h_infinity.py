...
class HInfinityController:
    ...
    def solve(self):
        ...
        S = self.B2 @ np.linalg.inv(self.R) @ self.B2.T - (1.0 / self.gamma**2) * (self.B1 @ self.B1.T)
        ...
        try:
            R_inv_eff = S
            self.P = solve_continuous_are(self.A, np.eye(self.A.shape[0]), self.Q, np.linalg.inv(R_inv_eff))
            return self.P
        except Exception as e:
            raise ValueError(f"H-infinity solution does not exist for gamma={self.gamma}. Try increasing gamma. Error: {e}")
