...
def cw_equations(
    r0: np.ndarray, v0: np.ndarray, n: float, t: float
) -> tuple[np.ndarray, np.ndarray]:
    """
...
def cw_targeting(r0: np.ndarray, r_target: np.ndarray, t: float, n: float) -> np.ndarray:
    ...
    A = np.array([[s / n, (2 / n) * (1 - c)], [(2 / n) * (c - 1), (4 * s / n - 3 * t)]])
...
def optimize_rpo_collocation(
    ...
    return {"success": res.success, "control": res.x.reshape((n_nodes, 3)), "message": res.message}
