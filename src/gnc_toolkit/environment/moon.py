...
class Moon:
    ...
    def calculate_moon_eci(self, jd):
        ...
        lon = (
            lam_m
            + 6.289 * np.sin(M_m_rad)
            - 1.274 * np.sin(M_m_rad - 2 * D_rad)
            + 0.658 * np.sin(2 * D_rad)
            + 0.214 * np.sin(2 * M_m_rad)
        )
        lat = (
            5.128 * np.sin(u_rad)
            + 0.280 * np.sin(u_rad + M_m_rad)
            + 0.277 * np.sin(M_m_rad - u_rad)
            + 0.173 * np.sin(u_rad - 2 * D_rad)
        )
        parallax = (
            0.9508
            + 0.0518 * np.cos(M_m_rad)
            + 0.0095 * np.cos(M_m_rad - 2 * D_rad)
            + 0.0078 * np.cos(2 * D_rad)
            + 0.0028 * np.cos(2 * M_m_rad)
        )
        ...
        y = r_mag * (
            np.cos(eps) * np.cos(lat_rad) * np.sin(lon_rad) - np.sin(eps) * np.sin(lat_rad)
        )
        z = r_mag * (
            np.sin(eps) * np.cos(lat_rad) * np.sin(lon_rad) + np.cos(eps) * np.sin(lat_rad)
        )
