...
def plan_avoidance_maneuver(r_sat: np.ndarray, v_sat: np.ndarray,
                           r_debris: np.ndarray, v_debris: np.ndarray,
                           safety_radius: float, t_encounter: float) -> Tuple[np.ndarray, float]:
    """
    ...
    if t_encounter < 10.0:
         dv_mag = delta_r_req / t_encounter # Impulse at encounter essentially doesn't work, requires high speed
    else:
    ...
    estimated_miss = current_miss + np.abs(dv_mag) * (3.0 * t_encounter if t_encounter > 10.0 else t_encounter)
