...
class ThirdBodyGravity:
    ...
    def get_acceleration(self, r_eci, jd):
        ...
        d_sun = r_sun - r_eci
        acc += self.mu_sun * (
            d_sun / np.linalg.norm(d_sun) ** 3 - r_sun / np.linalg.norm(r_sun) ** 3
        )

        # Moon contribution
        d_moon = r_moon - r_eci
        acc += self.mu_moon * (
            d_moon / np.linalg.norm(d_moon) ** 3 - r_moon / np.linalg.norm(r_moon) ** 3
        )
...
class RelativisticCorrection:
    ...
    def get_acceleration(self, r_eci, v_eci):
        ...
        a_sch = (self.mu / (self.c**2 * r_mag**3)) * (
            term1 * r_eci + 4 * np.dot(r_eci, v_eci) * v_eci
        )

        # Lense-Thirring (Frame dragging)
        # Standard formula: 2 * mu / (c^2 r^3) * [ (3/r^2) (r.S) r - S ] x v
        term_lt = (2 * self.mu / (self.c**2 * r_mag**3)) * (
            (3.0 / r_mag**2) * np.dot(r_eci, self.S_vec) * r_eci - self.S_vec
        )
